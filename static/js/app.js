...
(function (/** @type {any} */ aoc) {
  ...
  const largeLeaderboardCutOff = Math.trunc(
    (window.innerWidth < 1600 ? 25 : 40) / (isResponsivenessToggled() ? 2 : 1)
  );
  ...
  try {
    ...
    if (!presumedLoggedInUserName) {
      presumedLoggedInUserName = document
        .querySelector(".user")
        ?.textContent?.replace("(AoC++) ", "") // Individual sponsor marking
    ...
    console.info(
      "Could not reliably determine logged in user from AoC website html. Something may have changed in the HTML structure, or perhaps there's an edge case we've missed. Either way, we'll ignore this error and carry on."
    );
  }
  ...
  function getPalette(n, rainbow, original) {
    if (original) {
      const basePalette = [
        ...
        "rgba(96, 171, 157, 1.0)",
        "rgba(99, 173, 152, 1.0)",
        "rgba(102, 175, 147, 1.0)",
        "rgba(105, 177, 142, 1.0)",
        "rgba(108, 178, 137, 1.0)",
        "rgba(112, 180, 132, 1.0)",
        "rgba(115, 181, 128, 1.0)",
        "rgba(119, 182, 123, 1.0)",
        "rgba(122, 184, 119, 1.0)",
        "rgba(126, 185, 115, 1.0)",
        "rgba(130, 186, 111, 1.0)",
        "rgba(133, 186, 107, 1.0)",
        "rgba(137, 187, 104, 1.0)",
        "rgba(141, 188, 101, 1.0)",
        "rgba(145, 189, 97, 1.0)",
        "rgba(149, 189, 94, 1.0)",
        "rgba(153, 189, 92, 1.0)",
        "rgba(157, 190, 89, 1.0)",
    ...
    if (rainbow)
      ...
      return [...Array(n).keys()].map(
        (i) => "hsla(" + (i * 300) / n + ", 100%, 50%, 1.0)"
      );

    // Dynamic fire palette red->yellow->green using hsl()
    return [...Array(n).keys()].map(
      (i) => "hsla(" + (i * 120) / n + ", 100%, 50%, 1.0)"
    );
  }
  ...
  function transformRawAocJson(json) {
    ...
    let members = Object.keys(json.members)
      ...
      .map((m) => {
        ...
        loggedInUserIsPresumablyKnown =
          loggedInUserIsPresumablyKnown || m.isLoggedInUser;
        ...
        for (let dayKey of Object.keys(m.completion_day_level)) {
          for (let starKey of Object.keys(m.completion_day_level[dayKey])) {
            let starMoment = moment
              .unix(+m.completion_day_level[dayKey][starKey].get_star_ts)
              .utc();
            ...
            let /** @type {IStar} */ star = {
                ...
                getStarTimestamp:
                  m.completion_day_level[dayKey][starKey].get_star_ts,
        ...
        m.stars
          .filter((s) => s.starNr === 2)
          .forEach((star2) => {
            const star1 = m.stars.find(
              (s) => s.dayNr === star2.dayNr && s.starNr === 1
            );
            const deltaTimeTakenSeconds =
              star2.timeTakenSeconds - (star1?.timeTakenSeconds || 0);
            const delta = {
              dayNr: star2.dayNr,
              dayKey: star2.dayKey,
              deltaTimeTakenSeconds,
              member: m,
              points: 0,
            };
            deltas.push(delta);
        ...
        if (sortedDeltas.length > 0) {
          m.deltaMedianSeconds =
            sortedDeltas[
              Math.trunc(sortedDeltas.length / 2)
            ].deltaTimeTakenSeconds;
          m.deltaMeanSeconds =
            sortedDeltas
              .map((x) => x.deltaTimeTakenSeconds)
              .reduce((a, b) => a + b) / sortedDeltas.length;
        }
    ...
    let allMoments = stars
      .map((s) => s.getStarMoment)
      .concat([moment("" + year + "-12-25T00:00:00-0000")]);
    let maxMoment = moment.min([
      moment.max(allMoments),
      moment("" + year + "-12-31T00:00:00-0000"),
    ]);
    ...
    /** @type {number} */
    let maxDay = Math.max.apply(
      Math,
      stars.filter((s) => s.starNr === 2).map((s) => s.dayNr)
    );
    ...
    for (let d = 1; d <= maxDay; d++) {
      days[d] = {
        dayNr: d,
        podium: stars
          .filter((s) => s.dayNr === d && s.starNr === 2)
          .sort(starSorter),
        podiumFirstPuzzle: stars
          .filter((s) => s.dayNr === d && s.starNr === 1)
          .sort(starSorter),
      };
  ...
  function getPodiumFor(/** @type IMember */ member) {
    ...
    for (let p = 0; p < podiumLength; p++) {
      medals.push(
        member.stars.filter((s) => s.awardedPodiumPlace === p).length
      );
    }
    return medals;
  }
  ...
  function getPodiumForFirstPuzzle(/** @type IMember */ member) {
    ...
    for (let p = 0; p < podiumLength; p++) {
      medals.push(
        member.stars.filter((s) => s.awardedPodiumPlaceFirstPuzzle === p).length
      );
    }
    return medals;
  }

  function memberByPodiumSorter(
    /** @type IMember */ a,
    /** @type IMember */ b
  ) {
  ...
  function toggleResponsiveness() {
    localStorage.setItem(
      "aoc-flag-v1-is-responsive",
      !isResponsivenessToggled() + ""
    );
    location.reload();
  }
  ...
  function isResponsivenessToggled() {
    return !!JSON.parse(
      localStorage.getItem("aoc-flag-v1-is-responsive") || "null"
    );
  }
  ...
  function toggleCurrentGraphColorStyle({ shouldReload = true } = {}) {
    ...
    localStorage.setItem(
      "aoc-flag-v1-color-style",
      graphColorStyles[(cur + 1) % graphColorStyles.length]
    );
  ...
  function togglePointsOverTimeType() {
    ...
    localStorage.setItem(
      "aoc-flag-v1-points-over-time-type-index",
      value.toString()
    );
    location.reload();
  }
  ...
  function getPointsOverTimeType() {
    return (
      +(
        localStorage.getItem("aoc-flag-v1-points-over-time-type-index") || "0"
      ) || 0
    );
  }
  ...
  function getFullScreenSubject() {
    return (
      /** @type IFullScreenSubject */ (
        localStorage.getItem("aoc-flag-v1-full-screen-subject")
      ) || null
    );
  }
  ...
  function formatStarMomentForTitle(/** @type IStar */ memberStar) {
    return (
      memberStar.getStarMoment.local().format("HH:mm:ss YYYY-MM-DD") +
      " (local time)"
    );
  }
  ...
  function getLeaderboardJson() {
    ...
    if (cache) {
      ...
      if (Date.now() < ttl.getTime()) {
        ...
        return Promise.resolve(cache.data).then((json) =>
          transformRawAocJson(json)
        );
      }
    }
      ...
      this.plugins = {
        legend: {
          ...
          title: {
            ...
            // discoverable.
            text: data.isLargeLeaderboard
              ? `Only Showing Top ${largeLeaderboardCutOff}`
              : "(🖱 click / 🖱🖱 click)",
  ...
  class App {
    constructor() {
      ...
      this.perDayLeaderBoard = this.wrapper.appendChild(
    ...
    refreshFullScreenSetup() {
      ...
      switch (subject) {
        ...
        case "graphs":
          ...
          this.medals.classList.toggle(
            "aoc-extension-full-screen-subject",
            subject === "medals"
          );
          this.perDayLeaderBoard.classList.toggle(
            "aoc-extension-full-screen-subject",
            subject === "perDayLeaderBoard"
          );
          this.graphs.classList.toggle(
            "aoc-extension-full-screen-subject",
            subject === "graphs"
          );
          break;
        ...
        default:
          document.body.classList.remove(
            "aoc-extension-with-full-screen-overlay"
          );
          break;
      }
    }
    ...
    setupFullScreenModes(/** @type {IAppData} */ _data) {
      ...
      fullScreenWarning.innerText =
        "Note: viewing full-screen leaderboard generated by browser extension. Exit with the button top-right.";
      document.body.appendChild(fullScreenWarning);
    }

    loadControlButtons(/** @type {IAppData} */ data) {
      const cacheBustLink = this.controls.appendChild(
      ...
      const responsiveToggleLink = this.controls.appendChild(
        document.createElement("a")
      );
      responsiveToggleLink.innerText =
        (isResponsivenessToggled() ? "✅" : "❌") + " Graphs in 2x2 grid";
      responsiveToggleLink.title =
        "Trigger side-by-side graphs if the viewport is wider than 1800px";
      ...
      responsiveToggleLink.addEventListener("click", () =>
        toggleResponsiveness()
      );

      const colorToggleLink = this.controls.appendChild(
        document.createElement("a")
      );
      ...
      colorToggleLink.addEventListener("click", () =>
        toggleCurrentGraphColorStyle()
      );

      const fullScreenButtons = this.controls.appendChild(
        document.createElement("div")
      );
      fullScreenButtons.innerText = "| Go full screen with: ";
      fullScreenButtons.title =
        "Useful for example to show a permanent leaderboard in your office hallway on a monitor.";
    ...
    loadPerDayLeaderBoard(/** @type {IAppData} */ data) {
      ...
      let titleElement = this.perDayLeaderBoard.appendChild(
      ...
      if (data.maxDay <= 0) {
        let noDataText = this.perDayLeaderBoard.appendChild(
      ...
      if (displayDay !== "overview") {
        ...
        displayDay = displayDay
          ? Math.min(parseInt(displayDay), data.maxDay)
          : data.maxDay;
      }
      ...
      const overviewAnchor = titleElement.appendChild(
        document.createElement("a")
      );
      ...
      function generateOverviewTable() {
...

        {
          ...
          tr.appendChild(
            createHeaderCell(
              "Mean*",
              "#ffff66",
              "deltas of more than 4 hours are not included"
            )
          );
          tr.appendChild(
            createHeaderCell(
        ...
        for (let member of data.members
          .slice()
          .sort(deltaPointsTotalSorter)
          .reverse()) {
          const bgColor = member.isLoggedInUser
            ? aocColors["highlight"]
            : "transparent";
          rank += 1;
          let tr = table.appendChild(document.createElement("tr"));
          ...
          td.style.border = "1px solid #333";
          td.style.padding = "6px";
          td.style.backgroundColor = member.isLoggedInUser
            ? aocColors["highlight"]
            : "transparent";
          ...
          tr.appendChild(
            createCell(
              member.deltaMeanSeconds
                ? formatTimeTaken(member.deltaMeanSeconds)
                : "",
              bgColor
            )
          );
          ...
          for (let d = 1; d <= data.maxDay; ++d) {
            ...
            td.title =
              "Delta time: " +
              (delta ? formatTimeTaken(delta.deltaTimeTakenSeconds) : "none");
            td.style.padding = "2px";
            td.style.fontSize = "11px";
            td.style.background = `rgba(255,255,255,${
              delta ? (delta.points * delta.points) / divider / 10 : 0
            })`;
          }
        }
      ...
      function generateTable(displayDay) {
        ...
        function sortByDeltaTime(a, b) {
          let a1 = a.stars.find(
            (s) => s.dayNr === displayDay && s.starNr === 1
          );
          let a2 = a.stars.find(
            (s) => s.dayNr === displayDay && s.starNr === 2
          );
          let b1 = b.stars.find(
            (s) => s.dayNr === displayDay && s.starNr === 1
          );
        ...
        function sortByPart(starNr) {
          return function sortByPart2Time(a, b) {
            let aStar = a.stars.find(
              (s) => s.dayNr === displayDay && s.starNr === starNr
            );
            let bStar = b.stars.find(
              (s) => s.dayNr === displayDay && s.starNr === starNr
            );
            if (!aStar) return 1;
            if (!bStar) return -1;
        ...
        grid.sort(
          sortFns[
            Object.keys(sortFns).find((k) => k === getTimeTableSort()) ||
              "delta"
          ]
        );
...

        {
          ...
          th = tr.appendChild(
            createHeaderCell("part1", "----- Part 1 -----", "#9999cc")
          );
          th.colSpan = 3;
          th = tr.appendChild(createHeaderCell("delta", "----- Delta -----"));
          th.title =
            "Everyone starting puzzles at different times? See who's the fastest to go from 1 to 2 stars on a day!";
          th.colSpan = 1;
          th = tr.appendChild(
            createHeaderCell("part2", "----- Part 2 -----", "#ffff66")
          );
          th.colSpan = 3;
        ...
        }
        {
          ...
          td = tr.appendChild(
            createHeaderCell(
              "delta",
              "Delta Time" + (getTimeTableSort() === "delta" ? " ⬇" : "")
            )
          );
          ...
          td = tr.appendChild(
            createHeaderCell(
              "completion",
              "Points" + (getTimeTableSort() === "completion" ? " ⬇" : "")
            )
          );
        ...
        let rank = 0;
        let maxDeltaTime = Math.max.apply(
          Math,
          grid.map((m) => {
            let memberStar1 = m.stars.find(
              (s) => s.dayNr === displayDay && s.starNr === 1
            );
            let memberStar2 = m.stars.find(
              (s) => s.dayNr === displayDay && s.starNr === 2
            );
            const delta = memberStar2
              ? memberStar2.timeTakenSeconds -
                (memberStar1?.timeTakenSeconds || 0)
              : null;
            return delta || 0 > maxSecondsForSparkline ? null : delta;
          })
        );
        for (let member of grid) {
          let memberStar1 = member.stars.find(
            (s) => s.dayNr === displayDay && s.starNr === 1
          );
          let memberStar2 = member.stars.find(
            (s) => s.dayNr === displayDay && s.starNr === 2
          );
          ...
          let td = tr.appendChild(
            createCell(rank.toString() + ". " + member.name)
          );
          td.style.textAlign = "left";

          td = tr.appendChild(
            createCell(
              memberStar1 ? formatTimeTaken(memberStar1.timeTakenSeconds) : ""
            )
          );
          ...
          td = tr.appendChild(
            createCell(memberStar1 ? memberStar1.points : "")
          );
          ...
          if (memberStar2 && maxDeltaTime) {
            ...
            if (getTimeTableSort() === "delta") {
              sparkline.style.boxShadow =
                "1px 1px 5px rgba(255, 255, 255, 0.5), -1px -1px 5px rgba(255, 255, 255, 0.5)";
            }
            sparkline.style.opacity = delta > maxDeltaTime ? "0.15" : "0.75";
            sparkline.title =
              "Spark line showing relative 'delta time' values (up to a maximum delta time)";
          }

          td = tr.appendChild(
            createCell(
              memberStar2 ? formatTimeTaken(memberStar2.timeTakenSeconds) : ""
            )
          );
          ...
          td = tr.appendChild(
            createCell(memberStar2 ? memberStar2.points : "")
          );
    ...
    loadMedalOverview(/** @type {IAppData} */ data) {
      ...
      const medalFontFamily =
        '"Source Code Pro", monospace, serif, sans-serif, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji"';
      ...
      const showAllToggleLink = titleElement.appendChild(
        document.createElement("a")
      );
      showAllToggleLink.innerText = isShowAllToggled()
        ? "🎄 Showing all participants"
        : "🥇 Showing only medalists";
      showAllToggleLink.title =
        "Toggle between showing only medalists or all participants";
      ...
      for (let member of grid.sort(memberByPodiumSorter)) {
        const cellColor = member.isLoggedInUser
          ? aocColors["highlight"]
          : "transparent";
        ...
        for (let d = 1; d <= 25; d++) {
          ...
          if (d <= data.maxDay) {
            let secondPuzzlePodiumPlace = data.days[d].podium.findIndex(
              (n) => n.memberId === member.id
            );
            let firstPuzzlePodiumPlace = data.days[
              d
            ].podiumFirstPuzzle.findIndex((n) => n.memberId === member.id);

            if (
              firstPuzzlePodiumPlace >= 0 &&
              firstPuzzlePodiumPlace < podiumLength
            ) {
              div.style.boxShadow = `inset 2px 2px 0 0 ${medalColor(
                firstPuzzlePodiumPlace
              )}, inset -2px -2px 0 0 ${medalColor(firstPuzzlePodiumPlace)}`;
            ...
            let memberStar1 = member.stars.find(
              (s) => s.dayNr === d && s.starNr === 1
            );
            let memberStar2 = member.stars.find(
              (s) => s.dayNr === d && s.starNr === 2
            );
            ...
            if (
              secondPuzzlePodiumPlace >= 0 &&
              secondPuzzlePodiumPlace < podiumLength
            ) {
              medalCount++;
              div.style.opacity = `${
                0.5 +
                0.5 * ((podiumLength - secondPuzzlePodiumPlace) / podiumLength)
              }`;
            } else {
              span.innerText =
                secondPuzzlePodiumPlace >= 0
                  ? `${secondPuzzlePodiumPlace + 1}`
                  : "\u2003";
    ...
    loadDayVsTime(/** @type {IAppData} */ data) {
      let datasets = data.members.map((m) => {
        return {
          ...
          backgroundColor:
            data.isLargeLeaderboard && !m.isLoggedInUser
              ? m.colorMuted
              : m.color,
          borderWidth: 1,
          borderColor: "#000",
      ...
      let element = this.createGraphCanvas(
        data,
        "Log10 function of the time taken for each user to get the stars"
      );
      ...
      let chart = new Chart(element.getContext("2d"), {
        ...
        options: new ChartOptions(
          data,
          "Stars vs Log10(minutes taken per star)"
        )
          .withTooltips({
            callbacks: {
              label: (item) => {
    ...
    loadTimePerStar(/** @type {IAppData} */ data) {
      ...
      relevantMembers.forEach((member, idx) => {
        let star1DataSet = {
          ...
          borderWidth: 1,
          data: /** @type {number[]} */ ([]),
          hidden: data.loggedInUserIsPresumablyKnown
        ...
        for (let i = 1; i <= 25; i++) {
          ...
          let star2 = data.stars.find(
            (s) =>
              s.memberId === member.id && s.dayNr === i && s.starKey === "2"
          );

          star1DataSet.data.push(!!star1 ? star1.timeTaken : 0);
          star2DataSet.data.push(
            !!star2 ? star2.timeTaken - (star1?.timeTaken || 0) : 0
          );
        }
      ...
      let element = this.createGraphCanvas(
        data,
        "From the top players, show the number of minutes taken each day. (Exclude results over 4 hours.) (Toggle Responsive for all users)"
      );

      let options = new ChartOptions(data, `Minutes taken per star`).withYScale(
        {
          max: 240,
          ticks: {
            fontColor: aocColors["main"],
    ...
    loadPointsOverTime(/** @type {IAppData} */ data) {
      ...
      const maxPointsPerDay = Array.from(
        { length: maxDayNr },
        () => data.n_members * 2
      );
      ...
      data.stars.forEach(
        (s) =>
          (maxPointsPerDay[s.dayNr - 1] = s.points > 0 ? data.n_members * 2 : 0)
      );
      const availablePoints = [
        maxPointsPerDay.map((p) => p / 2),
        maxPointsPerDay.map((p) => p / 2),
      ];
      data.stars.forEach(
        (s) =>
          (availablePoints[s.starNr - 1][s.dayNr - 1] = Math.min(
            availablePoints[s.starNr - 1][s.dayNr - 1],
            Math.max(s.points - 1, 0)
          ))
      );

      let datasets = data.members
        .sort((a, b) => a.name?.localeCompare(b.name || "") || 0)
        .reduce((p, m) => {
          const days = m.stars.reduce((map, s) => {
          ...
          if (graphType === 2 && m.stars.length < maxPointsPerDay.length * 2) {
            p.push({
              ...
              showInLegend:
                m.isLoggedInUser || m.rank < largeLeaderboardCutOff / 2,
              order: m.isLoggedInUser ? 0 : 1, // lower one gets drawn on top
              lineTension: 0.1,
              fill: false,
              ...
              data: maxPointsPerDay
                ...
                .map((day, i, days) => ({
                  ...day,
                  pointsToDay: days
                    .filter((d) => d.dayNr <= day.dayNr)
                    .map((d) => d.points)
                    .reduce((a, b) => a + b),
                  maxPointsToDay: maxPointsPerDay
                    .slice(0, day.dayNr)
                    .reduce((p, max) => p + (max ?? 0), 0),
                }))
                .filter(
                  (day, i, days) =>
                    !days
                      .filter((d) => d.dayNr <= day.dayNr + 1)
                      .every((d) => d.stars.length === 2)
                )
                .map((day) => ({
          ...
          p.push({
            ...
            showInLegend:
              m.isLoggedInUser ||
              m.rank < largeLeaderboardCutOff / (graphType === 2 ? 2 : 1),
            order: m.isLoggedInUser ? 0 : 1, // lower one gets drawn on top
            lineTension: 0.1,
            fill: false,
            borderWidth: m.borderWidth,
            ...
            data:
              graphType !== 0
                ? maxPointsPerDay
                    ...
                    .map((day) => ({
                      x: moment([data.year, 10, 30, 0, 0, 0]).add(
                        day.dayNr,
                        "d"
                      ),
                      y: (day.pointsToDay / day.maxPointsToDay) * 100,
      ...
      const element = this.createGraphCanvas(
        data,
        "Points over time per member."
      );
      ...
      let chart = new Chart(element.getContext("2d"), {
        ...
        plugins: [
          {
            ...
            afterEvent: (chart, evt) => {
              ...
              const {
                titleBlock: { top, right, bottom, left },
              } = chart;
        ...
        options: new ChartOptions(
          data,
          `Points per Day - 🖱️ ${pointsOverTimeType[graphType]}`
        )
          .withTooltips({
            callbacks: {
              afterLabel: (item) => {
                if (graphType !== 0) {
                  ...
                  return `(day ${day.dayNr}. Total: ${day.pointsToDay} of ${
                    day.maxPointsToDay
                  } points. Today: ${day.points} points, ranked ${
                    day.stars.map((s) => `${s.rank}.`).join(" and ") || "-"
                  })`;
                }
    ...
    loadStarsOverTime(/** @type {IAppData} */ data) {
      ...
      let element = this.createGraphCanvas(
        data,
        "Number of stars over time per member."
      );
  ...
  if (
    ...
    console.info(
      `Loading via DOMContentLoaded because readyState = ${document.readyState}`
    );
